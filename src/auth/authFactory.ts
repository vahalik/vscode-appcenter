    ...
    public static async getAuth(authType: AuthProvider, logger: ILogger): Promise<AppCenterAuth | VstsAuth> {
        switch (authType) {
            case AuthProvider.Vsts: {
                const vstsProfileStorage: FsProfileStorage<VstsProfile> = new FsProfileStorage(
                    Utils.getVstsProfileFileName(),
                    logger,
                );
                const vstsAuth = new VstsAuth(vstsProfileStorage, logger);
                await vstsAuth.initialize();
                return vstsAuth;
            }
            case AuthProvider.AppCenter: {
                const appcenterProfileStorage: FsProfileStorage<AppCenterProfile> = new FsProfileStorage(
                    Utils.getAppCenterProfileFileName(),
                    logger,
                );
                const appCenterAuth = new AppCenterAuth(appcenterProfileStorage, logger);

...
export default class AppCenterAppBuilder {
    ...
    constructor(
        ...
        if (
            ...
            this.userOrOrg.displayName === undefined ||
                    ...
                    if (this.isCreatedForOrganization()) {
                        promises.push(
                            this.iOSAppCreator.createAppForOrg(
                                ...
                                AppCenterAppBuilder.getiOSDisplayName(this.projectName),
                                this.ownerName,
                            ),
                        );
                    } else {
                    ...
                    if (this.isCreatedForOrganization()) {
                        promises.push(
                            this.androidAppCreator.createAppForOrg(
                                ...
                                AppCenterAppBuilder.getAndroidDisplayName(this.projectName),
                                this.ownerName,
                            ),
                        );
                    } else {
    ...
    public async startProcess(): Promise<boolean> {
        return await VsCodeUI.showProgress(async (progress) => {
            ...
            if (this._createBetaTestersDistributionGroup) {
                ...
                if (
                    !createdBetaTestersGroup.every((val: boolean) => {
                        return val === true;
                    })
                ) {
            ...
            if (this._connectRepositoryToBuildService) {
                ...
                if (
                    ...
                    if (this._withBranchConfigurationCreatedAndBuildKickOff) {
                        ...
                        if (
                            !branchConfiguredAndBuildStarted.every((val: boolean) => {
                                return val === true;
                            })
                        ) {

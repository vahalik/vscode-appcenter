...
export async function getWindowsAppVersion(projectRoot?: string): Promise<string> {
    ...
    try {
        ...
        appxManifestContents = fs
            .readFileSync(path.join(appxManifestContainingFolder, appxManifestFileName))
            .toString();
    } catch (err) {
        throw new Error(
        ...
        xml2js.parseString(appxManifestContents, (err: Error, parsedAppxManifest: any) => {
            ...
            try {
                const appVersion: string =
                    parsedAppxManifest.Package.Identity[0]['$'].Version.match(/^\d+\.\d+\.\d+/)[0];
                console.log(
...
export function runReactNativeBundleCommand(
    ...
    Array.prototype.push.apply(reactNativeBundleArgs, [
        path.join(projectRootPath, 'node_modules', 'react-native', 'local-cli', 'cli.js'),
        'bundle',
        '--assets-dest',
        outputFolder,
        '--bundle-output',
...
export function getDefaultBundleName(os: string): string {
    ...
    return os.toLowerCase() === 'ios' ? 'main.jsbundle' : `index.${os.toLowerCase()}.bundle`;
}
